...
const getEmployeeIdCreating = (state: EmployeeState) => state?.creating;
const getEmployeeIdCreated = (state: EmployeeState) => state?.created;
...
export const selectEmployeeId = createSelector(selectFeature, (employeeState) :string | null =>{
  if(!!employeeState && employeeState.selectedEmployeeId){
      return employeeState.selectedEmployeeId
  }else{
    return null;
  }
});

export const selectEmployeeIdCreating = createSelector(selectFeature, getEmployeeIdCreating);
export const selectEmployeeIdCreated = createSelector(selectFeature, getEmployeeIdCreated);
